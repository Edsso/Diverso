...
// ---------------------- Leitura do Arquivo ----------------------
void carregar_entrada(string arquivo) {
    ifstream in(arquivo);
    string linha;
...
void chegada_dinamica() {
    for (auto& p : todos_processos) {
        if (p.chegada == tempo_global) {
            prontos.push(&p);
            cout << "[t=" << tempo_global << "] Processo " << p.id << " chegou\n";
        }
    }
}
...
void verificar_desbloqueios() {
    ...
    for (auto* p : bloqueados) {
        if (tempo_global - p->bloqueado_em >= p->espera) {
            ...
            cout << "[t=" << tempo_global << "] Processo " << p->id << " desbloqueado\n";
        }
    }
    ...
    for (size_t i = 0; i < nucleos.size(); i++) {
        ...
        if (!atual || atual->quantum_restante == 0 || atual->tempo_restante == 0) {
            if (atual && atual->tempo_restante > 0) {
                ...
                prontos.push(atual);
                cout << "[t=" << tempo_global << "] Processo " << atual->id << " preemptado\n";
            }
            if (!prontos.empty()) {
        ...
        if (p) {
            ...
            if (p->tempo_restante == 0) {
                if (p->bloqueia && !p->foi_bloqueado) {
                    ...
                    cout << "[t=" << tempo_global << "] Processo " << p->id << " bloqueado\n";
                } else {
                    p->tempo_fim = tempo_global + 1;
                    p->turnaround = p->tempo_fim - p->chegada;
                    cout << "[t=" << tempo_global << "] Processo " << p->id << " finalizado\n";
                }
                nucleos[i] = nullptr;
                p->em_execucao = false;
            }
